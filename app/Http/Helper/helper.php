...
if (!function_exists('ifHasPermissions')) {

     function ifHasPermissions($permission)
    {
        ...
        $role = $auth->roleUser()->first();
        if (!$role) {

            return false;
        }

        $hisPermissions = RoleHasPermission::where('role_id', $role->role_id)->get();
        $permission_id = Permission::where('name', $permission)->first()->id;
        $obj = $hisPermissions->where('permission_id', $permission_id)->first();
